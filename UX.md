...
- `edit_exercise.png`: The reference pack includes a richer exercise-detail screen tabbed into `Weight`, `Form`, `Progress`, and `History`. The provided screenshot shows `Weight`, with sets x reps, current/next working weight, progression settings, deload settings, and a plate helper. On the current branch, this deeper drill-down is not part of the shipped Programs flow; minimum slot editing remains local inside the workout editor, and the screenshot-only `Form` tab remains deferred and hidden.
